<template>
  <Row style="border-bottom: 1px solid #e2e2e2" class="flex" justify="center">
    <Col class="mainpage-col flex flex-col mx-14 mb-8" lg="12" sm="12" xs="12">
      <p class="main-title"><img width="150" src="/logo.png" /></p>
      <h4 class="sub-title">
        <b>Osen UI</b> – open-source Vue 3 components system for your next
        project based on TypeScript
      </h4>
      <div class="buttons-group">
        <router-link to="/start">
          <primary-btn type="primary" size="big" pulse>Get started</primary-btn>
        </router-link>
        <a
          target="_blank"
          rel="noopener noreferrer"
          href="https://github.com/osenco/ui/"
        >
          <primary-btn size="big" type="neutral" outlined>Github</primary-btn>
        </a>
      </div>
    </Col>
  </Row>
</template>

<style lang="less">
.button-logo {
  height: 15px;
  margin-right: 10px;
}
.main-title {
  font-size: 60px;
  margin-bottom: 12px;
  line-height: 1;
}

.sub-title {
  font-size: 17px;
  font-weight: normal;
  margin-bottom: 42px;
}

.buttons-group {
  display: flex;
  flex-direction: row;
  & a + a {
    margin-left: 22px;
  }
}

@media only screen and (max-width: 600px) {
  .mainpage-col {
    margin: 120px 30px 30px !important;
  }
}
</style>
