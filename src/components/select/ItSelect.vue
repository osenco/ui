<template>
  <div class="single-select">
    ...
    <div class="single-select-inner">
      ...
      >
        <span v-if="wrappedValue[trackBy]" class="single-select-selected">
          <slot name="selected-option" :props="props">
            {{ wrappedValue.name }}
          </slot>
        </span>

        <span v-else class="single-select-placeholder">
        ...
        <slot name="icon" :props="props">
          <i
            class="single-select-arrow material-icons"
            :class="show && 'single-select-arrow--active'"
          >
            unfold_more
          </i>
        </slot>
      ...
      <transition name="drop-bottom">
        <div v-if="show" class="single-select-dropdown" :class="dropdownClasses">
              ...
              <slot name="option" :props="props" :option="option">
                ...
                <transition name="fade-right">
                  <span
                    ...
                    class="single-select-option-check"
                  />
                </transition>
              </slot>
            </li>
...
export default defineComponent({
  ...
  setup(props: TProps, { emit, slots }) {
    ...
    const selectionClasses = computed(() => ({
      'single-select-selection--disabled': props.disabled,
      'single-select-selection--active': show.value,
    }))

    const dropdownClasses = computed(() => ({
      [props.placement
        ? `single-select-dropdown--${props.placement}`
        : `single-select-dropdown--${Positions.B}`]: true,
      'single-select-dropdown--divided': props.divided,
    }))
