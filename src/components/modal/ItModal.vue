    ...
    <transition name="fade">
      ...
      >
        <transition name="drop-top">
          <div v-show="modelValue" class="modal-wrap">
            <div class="modal-wrap-inner" @click.self="maskClick">
              <div
                class="modal-body"
                :class="{ 'modal-body--has-image': onlyImageSlot }"
                :style="!onlyImageSlot ? { maxWidth: width } : null"
              >
                <slot name="image" />
                <slot />
                <div v-if="itHasHeader" class="modal-header">
                  <slot name="header" />
                </div>
                <div v-if="itHasBody" class="modal-content">
                  <slot name="body" />
                </div>
                <div v-if="itHasActions" class="modal-footer">
                  <slot name="actions" />
                </div>
              </div>
            </div>
